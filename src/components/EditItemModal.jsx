...
import axios from "axios"; // Pastikan menggunakan axios atau fetch sesuai kebutuhan Anda
import { MdClose } from "react-icons/md";

export default function AddItemModal({ isOpen, onClose, itemId }) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                ...
                // Jika ada itemId, fetch data untuk mengedit
                if (itemId) {
                    const itemResponse = await axios.get(`http://127.0.0.1:8000/api/barang/${itemId}`);
                    const itemData = itemResponse.data;

                    // Set formData dengan data yang diterima
                    setFormData({
                        idBarang: itemData.id_barang,
                        idRak: itemData.id_rak,
                        idSupplier: itemData.id_supplier,
                        idAdmin: itemData.id_admin,
                        jumlahBarangMasuk: itemData.jumlah_barang_masuk,
                        expired: itemData.expired,
                        tanggalMasuk: itemData.tgl_masuk,
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        // Handle form submit logic here (POST or PUT based on action)
